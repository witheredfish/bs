...
def generate_resources_chart_data(allocations_count_by_resource_type):
    ...
    if allocations_count_by_resource_type:
        ...
        cloud_label = "云: %d" % (
            allocations_count_by_resource_type.get('Cloud', 0))
        server_label = "服务器: %d" % (
        ...
        resource_plot_data = {
            "columns": [
                [cluster_label, allocations_count_by_resource_type.get(
                    'Cluster', 0)],
                [storage_label, allocations_count_by_resource_type.get(
                    'Storage', 0)],
                [cloud_label, allocations_count_by_resource_type.get(
                    'Cloud', 0)],
                [server_label, allocations_count_by_resource_type.get(
                    'Server', 0)]
...
def generate_allocations_chart_data():
    ...
    renewal_requested_count = Allocation.objects.filter(
        status__name='Renewal Requested').count()
    ...
    active_label = "激活: %d" % (active_count)
    new_label = "新建: %d" % (new_count)
    renewal_requested_label = "续订请求: %d" % (
        renewal_requested_count)
    expired_label = "过期: %d" % (expired_count)
