...
from bs.core.project.models import (Project,
                                    ...
                                    ProjectUserMessage)
from bs.core.project.forms import (ProjectAddUserForm,
...
from bs.core.allocation.signals import allocation_activate_user, allocation_remove_user
from bs.core.publication.models import Publication
...
from bs.core.utils.common import get_domain_url, import_from_settings
from bs.core.utils.mail import send_email, send_email_template
    ...
    def get_context_data(self, **kwargs):
        ...
        for attribute in attributes_with_usage:
            try:
                ...
                logger.error("分配属性 '%s' 不是int类型但是有用法",
...
class ProjectUpdateView(SuccessMessageMixin, LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if project_obj.status.name not in ['Active', 'New', ]:
            messages.error(request, '你不能更新一个封存项目')
...
class ProjectAddUsersView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid() and allocation_form.is_valid():
            ...
            messages.success(
                request, '添加 {} 个人员'.format(added_users_count))
        else:
...
class ProjectRemoveUsersView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            messages.success(
                request, '删除 {} 个人员'.format(remove_users_count))
        else:
            for error in formset.errors:
                messages.error(request, error)
...
def project_update_email_notification(request):
    ...
    if request.method == "POST":
        ...
        if allowed == False:
            return HttpResponse('无许可', status=403)
        else:
            checked = data.get('checked')
            if checked == 'true':
                project_user_obj.enable_notifications = True
                project_user_obj.save()
                return HttpResponse('已确认', status=200)
            elif checked == 'false':
                project_user_obj.enable_notifications = False
                project_user_obj.save()
                return HttpResponse('未确认', status=200)
            else:
                return HttpResponse('无确认', status=400)
    else:
        return HttpResponse('没有POST请求', status=400)
...
class ProjectReviewView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        messages.error(
            self.request, '你没有权限审核项目')

    def dispatch(self, request, *args, **kwargs):
        project_obj = get_object_or_404(Project, pk=self.kwargs.get('pk'))

        if not project_obj.needs_review:
            messages.error(request, '你不需要审核该项目')
            return HttpResponseRedirect(reverse('project-detail', kwargs={'pk': project_obj.pk}))

        if '添加项目标题'.lower() in project_obj.title.lower():
            messages.error(
                request, '在审核项目之前，必须更新项目标题。')
            return HttpResponseRedirect(reverse('project-update', kwargs={'pk': project_obj.pk}))

        if '我们没有关于您的研究信息。请提供工作的详细描述并更新您的学科领域。谢谢！' in project_obj.description:
            messages.error(
                request, '在审核项目之前，必须更新项目标题。')
            return HttpResponseRedirect(reverse('project-update', kwargs={'pk': project_obj.pk}))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if project_review_form.is_valid():
            ...
            messages.success(request, '项目审核成功')
            return HttpResponseRedirect(reverse('project-detail', kwargs={'pk': project_obj.pk}))
        else:
            messages.error(
                request, '项目审核出错了')
...
class ProjectReviewCompleteView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, project_review_pk):
        ...
        messages.success(request, ' {} 的审核已完成'.format(
...
class ProjectReivewEmailView(LoginRequiredMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, '邮件已发给 {} ({})'.format(
...
class ProjectAttributeDeleteView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            messages.success(request, '删除{}个属性'.format(
...
class ProjectAttributeUpdateView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if project_obj.projectattribute_set.filter(pk=project_attribute_pk).exists():
            ...
            project_attribute_update_form = ProjectAttributeUpdateForm(
                request.POST, initial={'pk': self.kwargs.get('project_attribute_pk'), })
            ...
            if project_attribute_update_form.is_valid():
                ...
                messages.success(request, '属性已更新')
                return HttpResponseRedirect(reverse('project-detail', kwargs={'pk': project_obj.pk}))
            else:
