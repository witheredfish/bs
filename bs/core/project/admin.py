...
from bs.core.project.models import (Project, ProjectAdminComment,
...
class ProjectUserAdmin(SimpleHistoryAdmin):
    fields_change = ('user', 'project', 'role',
    ...
    def PI(self, obj):
        return '{} ({})'.format(obj.project.pi.first_name, obj.project.pi.username)
...
class ProjectAdmin(SimpleHistoryAdmin):
    fields_change = ('title', 'pi', 'description', 'status',
                     'requires_review', 'force_review', 'created', 'modified', )
    ...
    search_fields = ['pi__username', 'projectuser__user__username',
                     'projectuser__user__first_name', 'title']
    list_filter = ('status', 'force_review')
    inlines = [ProjectUserInline, ProjectAdminCommentInline,
               ProjectUserMessageInline, ProjectAttributeInLine]
    raw_id_fields = ['pi', ]

    def PI(self, obj):
        return '{} ({})'.format(obj.pi.first_name,  obj.pi.username)
...
class ProjectReviewAdmin(SimpleHistoryAdmin):
    list_display = ('pk', 'project', 'PI',
                    'reason_for_not_updating_project', 'created', 'status')
    search_fields = ['project__pi__username',
                     'project__pi__first_name',]
    list_filter = ('status', )
