...
class ProjectSearchForm(forms.Form):
    ...
    USERNAME = '用户名'
    FIELD_OF_SCIENCE = '学科领域'

    username = forms.CharField(label=USERNAME, max_length=100, required=False)
    field_of_science = forms.CharField(
        label=FIELD_OF_SCIENCE, max_length=100, required=False)
    show_all_projects = forms.BooleanField(
        initial=False, required=False, label="显示所有项目")
...
class ProjectAddUsersToAllocationForm(forms.Form):
    ...
    def __init__(self, request_user, project_pk, *args, **kwargs):
        ...
        if allocation_query_set:
            ...
            self.fields['allocation'].help_text = '<br/>将所选用户添加到的分配。'
        else:
...
class ProjectReviewForm(forms.Form):
    reason = forms.CharField(label='未更新项目信息的原因', widget=forms.Textarea(attrs={
                             'placeholder': '如果你没有要提供的新信息，需要在此框中解释这一点。谢谢！'}), required=False)
    acknowledgement = forms.BooleanField(
        label='选中此框，我已经尽我所知更新了我的项目', initial=False, required=True)
...
class ProjectReviewEmailForm(forms.Form):
    ...
    def __init__(self, pk, *args, **kwargs):
        ...
        self.fields['email_body'].initial = '你好， {} \n{}'.format(
            project_review_obj.project.pi.first_name, EMAIL_DIRECTOR_PENDING_PROJECT_REVIEW_EMAIL)
...
class ProjectAttributeAddForm(forms.ModelForm):
    class Meta:
        ...
        labels = {
            'proj_attr_type': "项目属性类型",
        }

    def __init__(self, *args, **kwargs):
        super(ProjectAttributeAddForm, self).__init__(*args, **kwargs)
        user = (kwargs.get('initial')).get('user')
        self.fields['proj_attr_type'].queryset = self.fields['proj_attr_type'].queryset.order_by(
            Lower('name'))
        if not user.is_superuser:
            self.fields['proj_attr_type'].queryset = self.fields['proj_attr_type'].queryset.filter(
                is_private=False)
