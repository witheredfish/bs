...
from bs.core.publication.forms import (
    PublicationAddForm,
...
from bs.core.publication.models import Publication, PublicationSource
...
class PublicationSearchResultView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def _search_id(self, unique_id):
        ...
        if 'journal' in bib_json:
            ...
            journal = '[{}没有期刊信息]'.format(source_name.upper())
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            if publications_added:
                msg += '给项目添加 {} 个出版物'.format(
                    publications_added, )
            if publications_skipped:
                msg += '项目已有这些出版物，未添加： {}'.format(
...
class PublicationDeletePublicationsView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if formset.is_valid():
            ...
            messages.success(request, '删除 {} 个出版物 '.format(
        ...
        if formset.is_valid():
            for form in formset:
                ...
                if publication_form_data['selected']:
                    ...
                    print("id 为"+publication_obj.display_uid())
