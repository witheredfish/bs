...
from bs.core.allocation.models import (
    AllocationAccount, AllocationAttributeType, AllocationAttribute, AllocationStatusChoice)
from bs.core.allocation.utils import get_user_resources
from bs.core.project.models import Project
from bs.core.resource.models import Resource, ResourceType
...
class AllocationForm(forms.Form):
    ...
    def __init__(self, request_user, project_pk,  *args, **kwargs):
        ...
        self.fields['resource'].queryset = get_user_resources(
            request_user).order_by(Lower("name"))
        ...
        if user_query_set:
            ...
            self.fields['users'].help_text = '<br/>选择项目人员来添加至这次分配'
        else:
        ...
        if ALLOCATION_ACCOUNT_ENABLED:
            ...
            self.fields['allocation_account'].help_text = '<br/>选择要与资源关联的帐户名称。 <a href="#Modal" id="modal_link">点此新建账户</a>'
        else:
            self.fields['allocation_account'].widget = forms.HiddenInput()

        self.fields['justification'].help_text = '<br/>请求这次分配的理由'
...
class AllocationSearchForm(forms.Form):
    project = forms.CharField(label='项目标题',
                              max_length=100, required=False)
    username = forms.CharField(
        label='用户名', max_length=100, required=False)
    ...
    allocation_attribute_value = forms.CharField(
        label='分配属性金额', max_length=100, required=False)
    end_date = forms.DateField(
        label='结束日期',
        widget=forms.DateInput(attrs={'class': 'datepicker'}),
        required=False)
...
class AllocationReviewUserForm(forms.Form):
    ALLOCATION_REVIEW_USER_CHOICES = (
        ('keep_in_allocation_and_project', '激活'),
        ('keep_in_project_only', '从这次分配中移除'),
...
class AllocationChangeForm(forms.Form):
    ...
    for choice in ALLOCATION_CHANGE_REQUEST_EXTENSION_DAYS:
        EXTENSION_CHOICES.append((choice, "{} 天".format(choice)))
