...
class LDAPUserSearch(UserSearch):
    ...
    def __init__(self, user_search_string, search_by):
        ...
        self.LDAP_SERVER_URI = import_from_settings(
            'LDAP_USER_SEARCH_SERVER_URI')
        self.LDAP_USER_SEARCH_BASE = import_from_settings(
            'LDAP_USER_SEARCH_BASE')
        self.LDAP_BIND_DN = import_from_settings(
            'LDAP_USER_SEARCH_BIND_DN', None)
        ...
        self.LDAP_USE_SSL = import_from_settings(
            'LDAP_USER_SEARCH_USE_SSL', True)

        self.server = Server(self.LDAP_SERVER_URI, use_ssl=self.LDAP_USE_SSL,
    ...
    def search_a_user(self, user_search_string=None, search_by='all_fields'):
        ...
        if user_search_string and search_by == 'all_fields':
            filter = ldap.filter.filter_format(
                "(|(givenName=*%s*)(sn=*%s*)(uid=*%s*)(mail=*%s*))", [user_search_string] * 4)
        elif user_search_string and search_by == 'username_only':
            filter = ldap.filter.filter_format(
                "(uid=%s)", [user_search_string])
            size_limit = 1
        else:
